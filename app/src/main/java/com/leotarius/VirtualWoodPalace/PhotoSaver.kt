        ...
        PixelCopy.request(arSceneView, bitmap, {result ->
            if(result == PixelCopy.SUCCESS){
                if(Build.VERSION.SDK_INT <= Build.VERSION_CODES.P){
                    ...
                    saveBitmapToGallery(bitmap, filename?: return@request)
                } else{
                    saveBitmapToGallery(bitmap)
                }
                activity.runOnUiThread{Toast.makeText(activity, "Photo saved successfully! ", Toast.LENGTH_SHORT).show()}
            } else{
                activity.runOnUiThread{Toast.makeText(activity, "Failed to take photo", Toast.LENGTH_SHORT).show()}
            }
